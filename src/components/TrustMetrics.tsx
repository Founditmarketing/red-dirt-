...
const TOTAL_HP_SOLD = 282_889;
const NEW_TRACTORS = 4_178;
const IMPLEMENTS = 15_692;

const TrustMetrics = () => {
    const ref = useRef<HTMLDivElement>(null);
    const inView = useInView(ref, { once: true, margin: '-80px' });
    const hpTotal = useCounter(TOTAL_HP_SOLD, 2400, inView);
    const tractors = useCounter(NEW_TRACTORS, 2000, inView);
    const implements_ = useCounter(IMPLEMENTS, 2000, inView);
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 text-center md:divide-x divide-white/20">
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.1 }} className="pb-8 md:pb-0 border-b md:border-none border-white/20">
                        <div className="text-4xl sm:text-5xl md:text-[64px] font-black tracking-tighter mb-1 md:mb-2 tabular-nums">
                            {tractors.toLocaleString()}
                        </div>
                        <div className="text-xs md:text-sm font-bold uppercase tracking-widest text-white/80">New Tractors Sold</div>
                    </motion.div>
